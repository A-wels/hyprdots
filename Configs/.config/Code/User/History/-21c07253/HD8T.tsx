...
const StartScreen = ({ navigation }) => {
    ...
    const loadPointsToWin = async () => {
      try {
        ...
        if (savedPointsToWin !== null) {
          ...
          console.log("savedPointsToWin", savedPointsToWin)
        }
      } catch (error) {
  ...
  const handlePointsToWin = (points: string) => {
    console.log("Trying to save points to win: ", points)
    if (points.trim() !== '') {
      // check if points is a number
      if (isNaN(parseInt(points))) {
        Alert.alert('Keine Zahl', 'Bitte gib eine Zahl ein!');
      } else {
        setPointsToWin(parseInt(points));
        savePointsToWin();
      }
    }
  };
    ...
    <View style={[styles.container]}>
      ...
      <View style={styles.inputContainerNumber}>
        ...
        <TextInput
          ...
          onChangeText={(text) => {setPointsToWinDisplay(text)}}
          // only save the points to win when the user is done typing
          onEndEditing={() => handlePointsToWin(pointsToWinDisplay)}
