...
pub fn load_timestamp_linux() -> Result<u32, Error> {
    ...
    let timestamp = contents.parse::<u32>();
    if timestamp.is_ok() {
...
pub fn load_subscriptions() -> Result<Vec<Subscription>, Error> {
    ...
    file.read_to_string(&mut contents)?;
    let subscriptions_read = serde_json::from_str(&contents);
    if subscriptions_read.is_err() {
        return Ok(vec![]);
    }
    let subscriptions: Vec<Subscription> = subscriptions_read.unwrap();
