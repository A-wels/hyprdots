...
use crate::structs::filesystem::{
    load_subscriptions, load_timestamp_linux, save_subscriptions, save_timestamp_linux,
};

use super::{subscription, Subscription, Token};
...
impl Server {
    ...
    pub fn new(url: String, protocol: String) -> Server {
        // Create a new client
        let client = reqwest::Client::new();

        // load the timestamp from the file
        let mut last_sync_loaded = load_timestamp_linux();
        let mut last_sync = 0;
        if last_sync_loaded.is_ok() {
            let last_sync = last_sync_loaded.unwrap();
        } else {
            let last_sync = 0;
        }

        let subscriptions_loaded = load_subscriptions();
        let mut subscriptions: Vec<Subscription> = vec![];
        if subscriptions_loaded.is_ok() {
            subscriptions = subscriptions_loaded.unwrap();
            println!("Loaded subscriptions from file");
            ...
            loop {
                ...
                if response.status().is_success() {
                    ...
                    if login_name != "" && app_password != "" {
                        ...
                        self.token.username = login_name.to_string();
                        let token = Token {
    ...
    pub async fn get_subscriptions(&mut self) -> Result<(), Error> {
        ...
        let timestamp = load_timestamp_linux();
        if timestamp.is_ok() {
            self.last_sync = timestamp.unwrap();
        } else {
            println!("No timestamp found. Syncing from the beginning");
        }

        // GET Request to the url
        let get_url = format!(
            "{}://{}/index.php/apps/gpoddersync/subscriptions?since={}",
            self.protocol, self.url, self.last_sync
        );
        let response = self
            .client
            .get(get_url)
            .basic_auth(&self.token.username, Some(&self.token.token))
            .send()
            .await?
            .text()
            .await?;
        ...
        // Store last sync time in a file using  save_timestamp_linux()
        let _ = save_timestamp_linux(self.last_sync.clone());
        ...
        let _ = save_subscriptions(self.subscriptions.clone());
        Ok(())
    }
}
