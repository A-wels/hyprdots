...
do
    ...
    elif flatpak_available ${pkg}
        then
        echo "queueing ${pkg} from flatpak..."
        pkg_flatpak=`echo $pkg_flatpak ${pkg}`
...
if [ `echo $pkg_flatpak | wc -w` -gt 0 ]
then
    echo "installing $pkg_flatpak from flatpak..."
    flatpak install -y $pkg_flatpak
fi
